...
func main() {
	...
	// maxTime := dayjs.Max(dayjs.Now(), dayTime, dayjs.Parse("2022年02月28"))
	// fmt.Println(maxTime)
	// minTime := dayjs.Min(dayjs.Now(), dayTime, dayjs.Parse("2022年02月28"))
	...
	// fmt.Println(dayTime.Clone().Add(-1, "date").Format())
	// fmt.Println(dayTime.EndOf("year").StartOf("month").Format())
